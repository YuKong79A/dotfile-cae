function f
    ...
    for arg in $argv
        ...
        if test "$arg" = -h; or test "$arg" = --help
            if test "$IS_ZH" = true
                ...
                echo "  fn      : NSFW mode. Equivalent to f --nsfw; please be aware of your surroundings."
    ...
    function get_random_url -V NSFW_MODE
        ...
        if test "$NSFW_MODE" = true
            set SOURCES waifu_im_nsfw
        else
            set SOURCES waifu_im_sfw nekos_best_waifu
        end

        set -l COUNT (count $SOURCES)
        set -l START (math (random) % $COUNT + 1)

        for offset in (seq 0 (math $COUNT - 1))
            set -l INDEX (math "($START + $offset - 1) % $COUNT + 1")
            set -l SOURCE $SOURCES[$INDEX]
            set -l URL

            switch $SOURCE
                case waifu_im_nsfw
                    set URL (curl -s -L $TIMEOUT "https://api.waifu.im/images?IncludedTags=waifu&IsNsfw=True&IsAnimated=False&PageSize=1" | jq -r '.items[0].url // empty' 2>/dev/null)
                case danbooru_nsfw
                    set URL (curl -s -L $TIMEOUT -H "User-Agent: f.fish/1.0" "https://danbooru.donmai.us/posts/random.json?tags=rating:e+-status:deleted+filetype:jpg" | jq -r '.file_url // .large_file_url // empty' 2>/dev/null)
                case waifu_im_sfw
                    set URL (curl -s -L $TIMEOUT "https://api.waifu.im/images?IncludedTags=waifu&IsNsfw=False&IsAnimated=False&PageSize=1" | jq -r '.items[0].url // empty' 2>/dev/null)
                case nekos_best_waifu
                    set URL (curl -s -L $TIMEOUT -H "User-Agent: f.fish/1.0" "https://nekos.best/api/v2/waifu" | jq -r '.results[0].url // empty' 2>/dev/null)
            end
